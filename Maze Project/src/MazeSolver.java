...
//remove this later on
	/**
	 * Solves the maze using DFS method if the user wanted to solve the maze themselves,
	 * then decided to quit before getting to the finish. 
	 * @param stack The stack of points the user had while they were solving the maze
	 * @param maze The maze the user was solving
	 *//*
	public static void userGaveUp(LinkedStack stack, char [][] maze)
	{
		System.out.println("Solving . . .");
		boolean solving = true;
		boolean whereUserLeftOff = true; //A boolean value used to keep the snowman where the user left off
		
		//Loop while the finish hasn't been found
		while(solving)
		{
			//Gets point at the top of stack, removes and store in a temporary point object
			Point temp = stack.pop();
			
			//Tests if the point is the finish, if it is exits the maze
			if(maze[(int) temp.getX()][(int) temp.getY()] == 'f')
			{
				solving = false;
			}
			
			//If not, tests the four points around the current position
			else
			{
				//If not right where the user left off, then marks current position with a dot
				if(!whereUserLeftOff)
				{
					maze[(int) temp.getX()][(int) temp.getY()] = '.';
				}
				whereUserLeftOff = false;
				
				//Tests the x value for bounds
				if(temp.getX() == 0)
				{
					//Means the current point is on the outer left edge and it can only test the point to the right
					if(maze[(int) temp.getX() + 1][(int) temp.getY()] != '*' 
						&& maze[(int) temp.getX() + 1][(int) temp.getY()] != '.')
					{
						//If the point to the right is not a wall or the computer hasn't been there already, it adds it to the stack
						stack.push(new Point(((int) temp.getX() + 1), (int) temp.getY()));
					}
				}
				
				//Tests if the current point is on the very right edge of maze
				else if(temp.getX() == maze.length - 1)
				{
					//If it is, can only test the point to the left of it (along the x axis)
					if(maze[(int) temp.getX() - 1][(int) temp.getY()] != '*' 
						&& maze[(int) temp.getX() - 1][(int) temp.getY()] != '.')
					{
						//If not a wall or hasn't been traveled, adds to stack
						stack.push(new Point(((int) temp.getX() - 1), (int) temp.getY()));
					}
				}
				
				//Otherwise it's not on the edges and can test the points on either side of current point
				else
				{
					//To the right
					if(maze[(int) temp.getX() + 1][(int) temp.getY()] != '*' 
				...
				//Tests the y value for bounds
				if(temp.getY() == 0)
				{
					//If at top of maze edge, can only test point below current position
					if(maze[(int) temp.getX()][(int) temp.getY() + 1] != '*' 
						&& maze[(int) temp.getX()][(int) temp.getY() + 1] != '.')
					{
						//Adds to stack if point below current position is not a wall and has not been traveled
						stack.push(new Point((int) temp.getX(), ((int) temp.getY() + 1)));
					}
				}
				
				//If point is at bottom edge of maze, tests only point above it
				else if(temp.getY() == maze[0].length - 1)
				{
					//Adds to stack if point above current position is not a wall and has not been traveled
					if(maze[(int) temp.getX()][(int) temp.getY() - 1] != '*' 
						&& maze[(int) temp.getX()][(int) temp.getY() - 1] != '.')
					{
						stack.push(new Point((int) temp.getX(), (int) temp.getY() - 1));
				...
				//Otherwise, the current position is not on the edge and can test both below and above current position
				else
				{
					//Below
					if(maze[(int) temp.getX()][(int) temp.getY() + 1] != '*' 
						&& maze[(int) temp.getX()][(int) temp.getY() + 1] != '.')
					{
						//Adds to stack if not a wall and has not been traveled
						stack.push(new Point((int) temp.getX(), ((int) temp.getY()+ 1)));
					}
					//Above
					if(maze[(int) temp.getX()][(int) temp.getY() - 1] != '*' 
